  ...
  //metoda za update podataka kad Shop zeli da izmeni podatke
    public function updateDataShop($idShop,$description, $shopName,$address,$image,$phoneNum,$name,$surname,$password) {
        ...
        if($password!="**********"){
            ...
            }else  $systemUserModel->update($idShop, ['name'=>$name,'surname'=>$surname,'phoneNum'=>$phoneNum,'password'=>$password]);
        }else  if($image!="newfileName"){$systemUserModel->update($idShop, ['name'=>$name,'surname'=>$surname,'phoneNum'=>$phoneNum,'image'=>$image]);}
                 else  $systemUserModel->update($idShop, ['name'=>$name,'surname'=>$surname,'phoneNum'=>$phoneNum]);
